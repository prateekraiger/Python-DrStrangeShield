# Enhanced Dr. Strange Shield System with Background Blur
import cv2
import time
...
class DrStrangeShieldWithBlur:
    ...
    def apply_background_blur(self, frame, blur_intensity=55):
        ...
        # Create mask (values between 0 and 1, where 1 = person, 0 = background)
        mask = results.segmentation_mask

        # Ensure blur intensity is odd for Gaussian blur
        ...
        # Apply moderate thresholding for better person detection
        # Values > 0.6 = definitely person, < 0.4 = definitely background
        person_threshold = 0.6
        background_threshold = 0.4

        # Create binary masks
        person_mask = mask_3channel > person_threshold
        background_mask = mask_3channel < background_threshold

        # Start with original frame
        output_frame = frame.copy()

        # Apply blur only to definite background areas
        output_frame = np.where(background_mask, blurred_frame, output_frame)

        # For intermediate values, blend gradually to avoid harsh edges
        intermediate_mask = (mask_3channel >= background_threshold) & (mask_3channel <= person_threshold)
        if np.any(intermediate_mask):
            # Gradual blending for edge areas
            blend_factor = (mask_3channel - background_threshold) / (person_threshold - background_threshold)
            blended = frame * blend_factor + blurred_frame * (1 - blend_factor)
            output_frame = np.where(intermediate_mask, blended, output_frame)
    ...
    def create_welcome_screen(self, width, height):
        ...
        # Subtitle
        subtitle = "With Background Blur & Magic Shields"
        ...
        instructions = [
            "✨ ENHANCED FEATURES:",
            "",
            "🎭 Background Blur (like Zoom/Teams)",
            "🛡️ Magical Shield Effects",
            "📸 Visual Gesture Instructions",
            "",
            "GESTURE SEQUENCE:",
            "1. Perform KEY_1 gesture",
            "2. Quickly perform KEY_2 gesture",
            "3. Quickly perform KEY_3 gesture",
            "4. Shields activate with blur!",
        ...
        for i, instruction in enumerate(instructions):
            ...
            if instruction.startswith("✨ ENHANCED FEATURES:"):
                color = (0, 255, 0)
    ...
    def create_status_overlay(self, frame, KEY_1, KEY_2, KEY_3, SHIELDS, current_gesture=None, confidence=0.0):
        ...
        if SHIELDS:
            # Clean view when shields are active - no overlay text needed
            # Let the user enjoy the pure magical shield effects with blur!
            pass
        else:
    ...
    def create_help_screen(self, width, height):
        ...
        # Title
        cv2.putText(screen, "GESTURE GUIDE + BACKGROUND BLUR", (width//2-200, 50),
        ...
        # Instructions
        cv2.putText(screen, "✨ Background automatically blurs when you're detected!", (50, height-60),
...
# Enhanced main function with background blur
def main():
    # Initialize UI with blur
    ui = DrStrangeShieldWithBlur()
    ...
    parser.add_argument("-b", "--blur", dest="blur_intensity", default=55, type=int,
                        help="Background blur intensity (higher = more blur). Default is 55")
    ...
    def signal_handler(sig, frame):
        ...
        print("\n\n" + "="*60)
        print("\n🛑 Shutting down Dr. Strange Shield System with Blur...")
        ...
        if ui.selfie_segmentation:
            ui.selfie_segmentation.close()
            print("  ✅ MediaPipe segmentation closed")
    ...
    if show_window:
        ...
        cv2.imshow('Dr. Strange Shield System with Blur', welcome_screen)

        # Wait for user to start
        print("\n" + "="*60)
        print("🛡️  DR. STRANGE SHIELD SYSTEM - WITH BACKGROUND BLUR")
        print("="*60)
        print("📺 Welcome screen displayed. Press SPACE to start or 'q' to quit")
        print(f"🎭 Background blur intensity: {args.blur_intensity}")
        print("="*60)
    ...
    print("🚀 System Ready! Starting gesture detection with background blur...")
        ...
        try:
            while cap.isOpened():
                ...
                if show_help:
                    ...
                    if show_window:
                        cv2.imshow('Dr. Strange Shield System with Blur', help_screen)
                ...
                # Store original frame for gesture detection
                ...
                # Make detection on ORIGINAL clear frame for better accuracy
                original_frame, results = mediapipe_detection(original_frame, holistic)
                xMinL, xMaxL, yMinL, yMaxL = get_center_lh(original_frame, results)
                xMinR, xMaxR, yMinR, yMaxR = get_center_rh(original_frame, results)

                # NOW apply background blur to the display frame
                frame = ui.apply_background_blur(frame, args.blur_intensity)
                ...
                if xMinL and xMinR and SHIELDS:
                    ...
                    if (prediction == 'key_4') and (pred_prob > 0.70):
                        ...
                        t2 = None
                        t3 = None
                        print(f"\n🛡️ SHIELDS DEACTIVATED! Perform KEY_1 to start sequence again.")
                    ...
                    if pred_prob > 0.7:  # Show when confidence is decent
                    ...
                    if (prediction == 'key_1') and (pred_prob > 0.75) and not KEY_1:
                        ...
                        if time_diff <= 5:  # Increased to 5 seconds for easier use
                            KEY_2 = True
                            KEY_3 = False  # Reset KEY_3
                            print(f"\n🔑 KEY_2 activated! ({time_diff:.1f}s) Perform KEY_3 within 5 seconds...")
                        else:
                            KEY_1 = False
                            KEY_2 = False
                            KEY_3 = False
                        ...
                        if time_diff <= 5:  # Increased time window
                            KEY_3 = True
                            SHIELDS = True
                ...
                # When shields are active, let user enjoy the clean magical view with blur
                if not SHIELDS:
                    if not KEY_1:
                ...
                # No instruction overlay when shields are active - clean blurred view!

                # Display frame
                if show_window:
                    # Resize frame for bigger display
                    display_frame = cv2.resize(frame, (int(width * 1.5), int(height * 1.5)))
                    cv2.imshow('Dr. Strange Shield System with Blur', display_frame)
